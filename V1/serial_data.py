# Code from https://www.youtube.com/watch?v=-SoEHsNKVpw
...
root = Tk()
root.title("TIPE 2022")
root.config(bg="grey")


def initComPort(index):
    currentPort = str(ports[index][0])
    print(currentPort)
    serialObj.port = currentPort
...
dataCanvas.grid(row=0, column=1, rowspan=len(ports))

vsb = Scrollbar(root, orient="vertical", command=dataCanvas.yview)
vsb.grid(row=0, column=2, rowspan=len(ports), sticky="ns")  # ns = north-south
